...
describe.skipIf(!integrationEnabled)('domain API integration', () => {
  ...
  it('persists, rotates, expires and revokes refresh sessions', async () => {
  ...
  it('covers ownership, page/version creation, pagination and immutable snapshots', async () => {
    ...
    const pageListResponse = await agent.get(`/api/v1/sites/${siteA}/pages`).expect(200);
  ...
  it('validates unsafe URLs and resource limits through the HTTP boundary', async () => {
    const suffix = randomUUID().slice(0, 8);
    const agent = request.agent(app.getHttpServer());
    await agent
      .post('/api/v1/auth/login')
      ...
      .expect(200);
    const sessionResponse = await agent.get('/api/v1/auth/me').expect(200);
    const workspaceId = (sessionResponse.body as { workspace: { id: string } }).workspace
      .id;
  ...
  it('keeps public delivery on the published immutable snapshot', async () => {
    const suffix = randomUUID().slice(0, 8);
    ...
    await agent
      ...
      .send({ expectedVersionNumber: 1, payload: payloadV2 })
      .expect(201);
    const draftResponse = await request(app.getHttpServer()).get(publicPath).expect(200);
    ...
    await request(app.getHttpServer()).get(`/api/v1/preview/pages/${pageId}`).expect(401);
  ...
  it('enforces the unique page version index at MongoDB level', async () => {
  ...
  it('accepts only published form schemas and keeps submissions scoped to that version', async () => {
    const suffix = randomUUID().slice(0, 8);
    const agent = request.agent(app.getHttpServer());
    await agent
      .post('/api/v1/auth/login')
