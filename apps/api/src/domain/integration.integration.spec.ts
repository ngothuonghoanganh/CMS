...
describe.skipIf(!integrationEnabled || env.INTEGRATION_EMAIL_PROVIDER !== 'fake')(
  ...
  () => {
    ...
    it('persists an enabled integration and delivers after submission', async () => {
      ...
      const site = await agent
        ...
        .send({ name: `Integration site ${suffix}`, slug: `integration-site-${suffix}` })
        .expect(201);
    ...
    it('enforces workspace scoping for integration APIs', async () => {
      const agent = request.agent(app.getHttpServer());
      await agent
        .post('/api/v1/auth/login')
        .send({ email: env.AUTH_EMAIL, password: env.AUTH_PASSWORD })
